...
def test_listado_roles():
    try:
        ...
        response = requests.get(f"{URL_BASE}/roles-remote", headers=headers)
        response.raise_for_status()
        datos = response.json()
...
def crear_rol():
    try:
        token = obtenerToken()
        headers = {"Authorization": f"Bearer {token}"}
        datos_rol = {"abilities": "home:R|profile:C|roles:R|", "description": "Rol de prueba", "name": "rpureba", "client_id": 3}
        response = requests.post(f"{URL_BASE}/roles-remote", json=datos_rol, headers=headers)
        response.raise_for_status()
        datos = response.json()
...
def test_llamar_rol(crear_rol):
    try:
        token = obtenerToken()
        headers = {"Authorization": f"Bearer {token}"}
        params = {"fullType": "DET", "client_id": 3, "searchBy": crear_rol}
...
def test_editar_rol(crear_rol):
    try:
        token = obtenerToken()
        headers = {"Authorization": f"Bearer {token}"}
        datos_edicion = {"description": "Prueba de Edicion", "client_id": 3}
...
def test_eliminar_rol(crear_rol):
    try:
        ...
        headers = {"Authorization": f"Bearer {token}"}
        params = {"client_id": 3}
        response = requests.delete(f"{URL_BASE}/roles-remote/{crear_rol}", headers=headers, params=params)
        response.raise_for_status()
        datos = response.json()
